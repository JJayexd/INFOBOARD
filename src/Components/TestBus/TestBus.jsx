  ...
  useEffect(() => {
    const fetchDepartures = async () => {
      try {
        ...
        console.log("Data fra Rejseplanen:", data);
    ...
    const intervalId = setInterval(fetchDepartures, 600000);
        ...
        {children || (
          <>
            <ul className="space-y-2">
              {departures.slice(0, 7).map((departure, index) => (
                <li key={index} className="m-1">
                  <p>
                    <strong>{departure.name}</strong> går mod{' '}
                    <strong>{departure.direction}</strong>
                  </p>
                  <p>
                    <strong>Kører</strong> {departure.time}
                  </p>
                </li>
              ))}
            </ul>
